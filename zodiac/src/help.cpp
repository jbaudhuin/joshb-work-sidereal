...
void
HelpWidget::loadArticles(QString path, quint16 maxArticleLength)
{
    int loadedCount = 0;
    ...
    qDebug("HelpWidget: loaded %d/%lld articles from %s less than %d bytes in "
           "%2.1fs",
           loadedCount,
           articles.count(),
           qPrintable(path),
           maxArticleLength,
           startTime.msecsTo(QTime::currentTime()) * 1.0 / 1000);
}

void
HelpWidget::setContent(Article& article)
{
    if (!article.loaded) {
...
void
HelpWidget::setContent(const QString& title, const QString& text)
{
...
QString
HelpWidget::getTag(QString searchStr)
{
...
void
HelpWidget::searchFor(QString key)
{
...
void
HelpWidget::setAlignment(Qt::Alignment a)
{
